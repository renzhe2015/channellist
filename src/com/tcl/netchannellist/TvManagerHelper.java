...
import com.tcl.netchannellist.bean.ChannelItemBean;
import com.tcl.netchannellist.bean.ColumnMode;
...
public class TvManagerHelper {
	...
	// DTV manual tuning
	// public static final int TUNING_DTV_BANDWIDTH_8MHZ =
	// TvManager.TUNING_BANDWIDTH_8MHZ;
	// public static final int TUNING_DTV_BANDWIDTH_7MHZ =
	// TvManager.TUNING_BANDWIDTH_7MHZ;
	// public static final int TUNING_DTV_BANDWIDTH_7MHZ_8MHZ =
	...
	// 数字音频输出
	public static final int AUDIO_DIGITAL_RAW = 0;
	public static final int AUDIO_DIGITAL_LPCM_DUAL_CH = 1;
	// 声道
	...
	public static class ChannelInformation {
		...
		ChannelInformation(ColumnMode mode) {
			tvSystemType = ChannelInfo.RT_TV_SYSTEM_UNKNOWN;
			sourceType = 11;// ?
			channelName = mode.getTitle();
			channelNumber = 0;
			packetid = mode.getPacketid();
			categoryid = mode.getCategoryid();
			DbId = mode.getDbID();
			channelIndex = 0;
			channelInfo = null;
			skipped = false;
		...
		ChannelInformation(ChannelItemBean bean) {
			tvSystemType = ChannelInfo.RT_TV_SYSTEM_UNKNOWN;
			sourceType = 12;// ?
			channelName = bean.getChannelName();
			channelNumber = Integer.valueOf(bean.getChannelNumber());
		...
		public String toString() {
			return String.format(Locale.US, "%03d %s", channelNumber,
					channelName);
		}
	...
	public static class ChannelSchedule extends ChannelInformation {
		...
		/**
		 * Get the date of this schedule. The time will be set to the begin of
		 * that day.
		 * 
		 * @return The date of this schedule or {@code null} if no data
		 *         available.
		 */
	...
	public static class TvProgram {
		...
		TvProgram(EpgData epg) {
			...
			this.epg = epg;
			if (VERBOSE) {
				DebugUtil.v(TAG, "TvProgram: " + toString());
			}
		}

		@Override
		public String toString() {
			return String.format("%s: %s ~ %s", title, startTime.toString(),
					endTime.toString());
		}
	...
	private TvManagerHelper(Context context) {
		...
		mPreference = context.getSharedPreferences("tv_manager",
	...
	/**
	 * Setup input source. It will execute asynchronously for the TvManager
	 * blocks too long.
	 * 
	 * @param tm
	 * @param src
	...
	public void setCurrentChannel(final int channelNumber,
			final Runnable callback, final Looper looper) {
		long time;
		if (BENCHMARK) {
		...
		if (BENCHMARK) {
			...
			DebugUtil.d(TAG, "setCurrentChannel: " + time);
		}
	}

	private Runnable mPostSetCurrentChannel;

	public void postSetCurrentChannel(long delay, final int channelNumber,
			final Runnable callback, final Looper looper) {
	...
	public void setCurrentChannelByIndex(final int channelIndex,
			final Runnable callback, final Looper looper) {
		if (!hasChannel(getInputSource())) {
			return;
		}
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				if (VERBOSE) {
					DebugUtil.v(TAG, String.format(
							"setCurrentChannelByIndex: channel=%d",
							channelIndex));
				}
				...
				// } else {
				mTvManager.playChannelByIndex(channelIndex);
				// mTvManager.setCurrentChannel(channelNumber,
				// mRunUpdateChannel);
				// }
				if (callback != null) {
					Handler h = new Handler(looper);
	...
	public List<ChannelSchedule> getChannelSchedules(int dayOffset) {
		...
		if (channelCount > 0) {
			...
			while (start < channelCount) {
				...
				ChannelInfo[] ci = mTvManager.getChInfoArray(filter, start,
						channelCount - start);
				if (ci != null) {
					if (ci.length == 0) {
						break;
					...
					for (int i = 0; i < ci.length; i++) {
						list.add(getChannelSchduleByChannelInfo(ci[i],
								dayOffset));
					}
					start += ci.length;
				} else {
					DebugUtil.w(TAG, "Null response from getChInfoArray");
					break;
				}
			}
		}
	...
	public List<ChannelInformation> getNetworkChannelList(Context mContext) {
		List<ChannelInformation> list = new ArrayList<ChannelInformation>();
		list.addAll(getCategroyChannelList(mContext));
		list.addAll(getAppChannelList(mContext));
		DebugUtil.d("cedar", "list.size()=" + list.size());
		return list;
	}

	public List<ChannelInformation> getCategroyChannelList(Context mContext) {
		List<ChannelInformation> list = new ArrayList<ChannelInformation>();
		Cursor cursor = mContext.getContentResolver()
				.query(TableColumn.CONTENT_URI, null,
						TableColumn.COLUMN_NAME_TYPE + "=?",
						new String[] { "5" }, null);
		if (cursor != null) {
			while (cursor.moveToNext()) {
				ColumnMode mColumnMode = ColumnMode
						.convertModeFromCursor(cursor);
				ChannelInformation info = new ChannelInformation(mColumnMode);
				list.add(info);
			}
			cursor.close();
		}
	...
	public List<ChannelInformation> getAppChannelList(Context mContext) {
		List<ChannelInformation> list = new ArrayList<ChannelInformation>();
		Cursor cursor = mContext.getContentResolver().query(
				Uri.parse(ChannelContract.AUTHORITY_URI + "/channel"),
				new String[] { ChannelItem.SUB_ID, ChannelItem.POSTER_URL,
						ChannelItem.CHANNEL_INSTALLED,
						ChannelItem.CHANNEL_NAME, ChannelItem.CHANNEL_NUMBER,
						ChannelItem.CHANNEL_PACKAGE_NAME,
						ChannelItem.CHANNEL_TYPE,
						ChannelItem.CHANNEL_TYPE_TEXT,
						ChannelItem.CHANNEL_VISIBLE,
						ChannelItem.COLUMN_FAVORITE },
				ChannelItem.CHANNEL_VISIBLE + "=? OR "
						+ ChannelItem.CHANNEL_INSTALLED + "=? ",
				new String[] { "1", "1" }, null);
		if (cursor != null) {
			while (cursor.moveToNext()) {
				ChannelItemBean bean = ChannelItemBean
						.convertChannelItemFromCursor(cursor);
				ChannelInformation info = new ChannelInformation(bean);
				list.add(info);
			}
			cursor.close();
		}
	...
	private void getChannelInfoArray(List<ChannelInformation> list,
			int channelCount, ChannelFilter filter) {
		int start = 0;
		while (start < channelCount) {
			// This returns an array with maximum length 25.
			ChannelInfo[] ci = mTvManager.getChInfoArray(filter, start,
	...
	public List<ChannelInfo> _getChannelList() {
		...
		if (channelCount > 0) {
			ChannelFilter filter = mTvManager.getDefaultFilter();
			ChannelInfo[] ci = mTvManager.getChInfoArray(filter, 0,
					channelCount);
			if (ci != null) {
				for (int i = 0; i < ci.length; i++) {
					list.add(ci[0]);
	...
	public ChannelInformation getCurrentChannelInformation() {
		...
		if (hasChannel(getInputSource())) {
			ChannelInfo ci = null;
			ChannelFilter filter = mTvManager.getDefaultFilter();
			ChannelInfo[] array = mTvManager.getChInfoArray(filter,
					getCurrentChannelIndex(), 1);
			if (array != null && array.length > 0) {
				ci = array[0];
				if (BENCHMARK) {
					time = SystemClock.uptimeMillis() - time;
					DebugUtil.d(TAG, "getCurrentChannelInformation: " + time);
				}
				ChannelInformation info = new ChannelInformation(ci);
				if (VERBOSE) {
					DebugUtil.d(TAG,
							"getCurrentChannelInformation: " + info.toString());
				}
				return info;
			}
		}
	...
	public ChannelInformation getChannelAt(int channelIdx) {
		if (hasChannel(getInputSource())) {
			ChannelInfo ci = null;
			ChannelFilter filter = mTvManager.getDefaultFilter();
			ChannelInfo[] array = mTvManager.getChInfoArray(filter, channelIdx,
					1);
			if (array != null && array.length > 0) {
				ci = array[0];
	...
	public ChannelSchedule getCurrentChannelSchedule() {
		if (hasProgramInformation()) {
			...
			ChannelFilter filter = mTvManager.getDefaultFilter();
			ChannelInfo[] array = mTvManager.getChInfoArray(filter,
					mTvManager.getCurChannelIndex(), 1);
			if (array != null && array.length > 0) {
				ci = array[0];
	...
	public ChannelSchedule getChannelSchduleByChannelInfo(ChannelInfo ci,
			int dayOffset) {
		EpgData[] epgs = mTvManager.getEpgDailyListByChIdx(ci.m_Channelndex,
				dayOffset);
		if (VERBOSE) {
			DebugUtil.v(TAG, String.format(
					"API: getEpgDailyListByChIdx(%d,%d): length = %d",
	...
	public static boolean isAtvSource(int source) {
		return source == TvManager.SOURCE_ATV1
				|| source == TvManager.SOURCE_ATV2;
	}
	...
	public static boolean isYpbprSource(int source) {
		return source == TvManager.SOURCE_YPP1
				|| source == TvManager.SOURCE_YPP2;
	}

	public static boolean isDtvSource(int source) {
		return source == TvManager.SOURCE_DTV1
				|| source == TvManager.SOURCE_DTV2;
	}
	...
	public static boolean isTvSource(int source) {
		return isAtvSource(source) || isDtvSource(source)
				|| isIdtvSource(source);
	}

	public static boolean isPCSource(int source) {
		return source == TvManager.SOURCE_VGA1
				|| source == TvManager.SOURCE_VGA2;
	}
	 ...
	 * 
	 * @param atvSoundStd
	 *            See {@link TvManager#RT_ATV_SOUND_SYSTEM_M}
	 */
	...
	public void setCurrentAtvFrequencyOffset(final int offset,
			final boolean perminant) {
	...
	public int getCurrentAtvFrequencyOffset() {
		 ...
		 * RpcExecutorManager::Execute(): ??????????? ERROR: Cannot find
		 * function(getTvCurChFreqOffset) RpcClientThread::ThreadProc(): ?????
		 * Invalid result(-1)
		 */
	...
	public void setAtvChannelBooster(int channelIndex, boolean enabled) {
		/*
		 * TODO: Doesn't work. RpcExecutorManager::Execute(): ??????????? ERROR:
		 * Cannot find function(setChannelbooster)
		 * RpcClientThread::ThreadProc(): ????? Invalid result(-1)
		 * RpcExecutorManager::Execute(): ??????????? ERROR: Cannot find
		 * function(setNoiseReduction_impulse) RpcClientThread::ThreadProc():
		 * ????? Invalid result(-1)
		 */
	...
	/**
	 * Get the size of frequency table. Note that this is only available when
	 * {@link #isAtvChannelBoosterEnabled(int)} is true.
	 * 
	 * @return
	 */
	...
	/**
	 * Lookup the ATV frequency in the table. Note that this is only available
	 * when {@link #isAtvChannelBoosterEnabled(int)} is true.
	 * 
	 * @param tableIndex
	 *            zero-based index according to the table size returned by
	 *            {@link #getAtvFrequencyTableSize()}
	 * @return
	 */
	 ...
	 *            Useless
	 */
	public void startDtvScanning(int frequency, int bandwidth, int index,
			int source) {
	...
	public void set3DMode(int mode) {
		...
		mPreference.edit().putInt(KEY_PREF_3DMODE, mTvManager.get3dMode())
				.commit();
	}
	...
	public static boolean is3Dto2DOptionEnabled(int mode) {
		return mode == MODE_3D_AUTO || mode == MODE_3D_LEFT_RIGHT
				|| mode == MODE_3D_UP_DOWN;
	}
